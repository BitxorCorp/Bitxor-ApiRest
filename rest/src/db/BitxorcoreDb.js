...
const { EntityType } = bitxorcore.model;
...
class BitxorcoreDb {
	 ...
	 * `height`, `fromHeight`, `toHeight`, `embedded`, `transferTokenId`, `fromTransferAmount`, `toTransferAmount`, `transactionTypes`
	 *  array of uint.
	...
	async transactions(group, filters, options) {
		...
		const buildConditions = async () => {
			...
			if (undefined !== filters.transferTokenId)
				conditions['transaction.tokens.id'] = convertToLong(filters.transferTokenId);

			if (undefined !== filters.fromTransferAmount || undefined !== filters.toTransferAmount) {
				const amountPath = 'transaction.tokens.amount';
	 ...
	 * @param {uint64} tokenId Filters by accounts with some tokenId balance. Required if provided `sortField` is `balance`
	 * @param {object} options Options for ordering and pagination. Can have an `offset`, and must contain the `sortField`, `sortDirection`,
	 * `pageSize` and `pageNumber`. 'sortField' must be within allowed 'sortingOptions'.
	 * @returns {Promise.<object>} Accounts page.
	 */
	accounts(address, tokenId, options) {
		const sortingOptions = { id: '_id', balance: 'account.tokens.amount' };
		let conditions = {};
		...
		if (undefined !== tokenId)
			conditions['account.tokens.id'] = convertToLong(tokenId);
		...
		if ('balance' === options.sortField) {
			...
			// fetch result sorted by specific token amount, this unwinds tokens and only returns matching tokens (incomplete response)
			queryPromise = this.database.collection('accounts')
				.aggregate([], { promoteLongs: false })
				.unwind('$account.tokens')
				.match(conditions)
				.sort(sortConditions)
				.skip(pageSize * pageIndex)
				.limit(pageSize)
				...
				.then(accounts => {
					...
					// repeat the response with the found and sorted account ids, so that the result can be complete with all the tokens
	...
	accountsByIds(ids) {
		...
		const buffers = ids.map(id => Buffer.from((id.publicKey
			? bitxorcore.model.address.publicKeyToAddress(id.publicKey, this.networkId) : id.address)));
