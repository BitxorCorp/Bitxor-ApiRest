...
module.exports = {
	register: (server, db, services) => {
		...
		server.get('/transactions/:group', (req, res, next) => {
			...
			if ((params.fromTransferAmount || params.toTransferAmount) && !params.transferTokenId)
				return next(new InvalidArgumentError('can\'t filter by transfer amount if `transferTokenId` is not provided'));
			...
			const filters = {
				...
				transferTokenId: params.transferTokenId ? routeUtils.parseArgument(params, 'transferTokenId', 'uint64hex') : undefined,
