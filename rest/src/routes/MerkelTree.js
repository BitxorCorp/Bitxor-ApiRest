...
class MerkleTree {
	...
	getBranchHash(encodedPath, links) {
		const branchLinks = Array(16).fill(bitxorcore.utils.convert.uint8ToHex(new Uint8Array(32)));
		links.forEach(link => {
			branchLinks[parseInt(`0x${link.bit}`, 16)] = link.link;
		});
		return bitxorcore.crypto.sha3Hasher.getHasher(32).update(
			bitxorcore.utils.convert.hexToUint8(encodedPath + branchLinks.join(''))
		)
			.hex()
			.toUpperCase();
	}
	...
	getLeafHash(encodedPath, leafValue) {
		return bitxorcore.crypto.sha3Hasher.getHasher(32).update(
			bitxorcore.utils.convert.hexToUint8(encodedPath + leafValue)
		)
			.hex()
			.toUpperCase();
	}
