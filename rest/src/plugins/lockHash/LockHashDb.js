...
class LockHashDb {
	/**
	 * Creates LockHashDb around BitxorcoreDb.
	...
	hashLocks(addresses, options) {
		...
		return this.bitxorcoreDb.queryPagedDocuments(conditions, [], sortConditions, 'hashLocks', options);
	}
	...
	hashLockByHash(ids) {
		...
		const collection = this.bitxorcoreDb.database.collection('hashLocks');
