...
const namespaceUtils = {
	 ...
	 * @param {module:db/BitxorcoreDb} bitxorcoreDb Bitxorcore database.
	...
	aliasNamesRoutesProcessor: (
		...
		return bitxorcoreDb.activeNamespacesWithAlias(aliasType, ids).then(namespaces => {
			const namespaceIds = [];
			...
			return bitxorcoreDb.registerNamespaceTransactionsByNamespaceIds(namespaceIds).then(transactions => {
