...
class ReceiptsDb {
	/**
	* Creates ReceiptsDb around BitxorcoreDb.
	...
	*/
	...
	transactionStatements(filters, options) {
		...
		if (undefined !== filters.artifactId) {
			const artifactIdType = isNamespaceId(filters.artifactId) ? 'namespaceId' : 'tokenId';
		...
		return this.bitxorcoreDb.queryPagedDocuments(conditions, [], sortConditions,
	...
	async addBlockMeta(page) {
		const data = await this.bitxorcoreDb.addBlockMetaToEntityList(page.data, ['timestamp'], item => item.statement.height);
