...
const bitxorcoreConnection = require('./bitxorcoreConnection');
const errors = require('../server/errors');
const tls = require('tls');

/**
 * Creates a bitxorcore connection service for connecting to bitxorcore servers.
 ...
 * @returns {object} Bitxorcore connection service.
 */
module.exports.createConnectionService = (config, logger = () => {}) => {
	...
	/**
	 * Opens a new connection authenticated to bitxorcore.
			...
			serverSocket
				.once('secureConnect', () => {
					if (serverSocket.authorized) {
						// wrap the socket in a bitxorcore connection and save it
						const serverConnection = bitxorcoreConnection.wrap(serverSocket);
						if (isPersistent) {
	...
	return {
		 ...
		 * @returns {module:connection/bitxorcoreConnection~BitxorcoreConnection} A connection.
		 */
		lease: () => {
