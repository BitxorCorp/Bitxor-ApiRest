...
const BitxorcoreDb = require('./db/BitxorcoreDb');
...
const loadConfig = () => {
	...
	configFiles.forEach(configFile => {
		...
		if (config) {
			// override config
			bitxorcore.utils.objects.checkSchemaAgainstTemplate(config, partialConfig);
			bitxorcore.utils.objects.deepAssign(config, partialConfig);
		} else {
			// primary config
...
const connectToDbWithRetry = (db, config) => bitxorcore.utils.future.makeRetryable(
...
const createServer = config => {
	const modelSystem = bitxorcore.plugins.bitxorcoreModelSystem.configure(config.extensions, {
...
(() => {
	...
	const network = bitxorcore.model.networkInfo.networks[config.network.name];
	if (!network) {
