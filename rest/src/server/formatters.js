...
const { formatArray, formatPage } = bitxorcore.utils.formattingUtils;

const isBitxorcoreObject = body => body && body.payload && body.type;

const formatBody = (modelFormatter, body) => {
	const formatBitxorcoreObject = (payload, type, structure, wsTopic) => {
	...
	} else if (isBitxorcoreObject(body)) {
		view = formatBitxorcoreObject(body.payload, body.type, body.structure, body.topic);
	}
...
module.exports = {
	...
	create: modelFormatters => ({
		 ...
		 */
		ws: body => (isBitxorcoreObject(body) && 'raw' === body.type ? body.payload : formatBody(modelFormatters.ws, body).json)
	})
};
