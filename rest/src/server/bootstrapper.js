...
const bitxorcoreRestifyPlugins = {
...
module.exports = {
	...
	createServer: (config, formatters, throttlingConfig) => {
		...
		server.pre(bitxorcoreRestifyPlugins.body());
		...
		server.use(bitxorcoreRestifyPlugins.crossDomain(addCrossDomainHeaders));
		...
		const promiseAwareServer = {
			listen: port => {
				// sort routes by route name in descending order (bitxorcore is only using string routes) in order to ensure that
		...
		server.on('MethodNotAllowed', (req, res) => {
			if ('OPTIONS' === req.method) {
				// notice that headers need to be added explicitly because bitxorcoreRestifyPlugins.crossDomain is not called after errors
				addCrossDomainHeaders(req, res);
				return res.send(204);
			}
