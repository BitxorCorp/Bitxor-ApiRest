...
describe('account routes', () => {
	...
	describe('GET', () => {
		describe('accounts', () => {
			const testTokenId = 'ABCDEF0123456789';
			...
			it('forwards query without tokenId if not provided', () => {
				// Arrange:
				const req = { params: {} };
			...
			it('throws error if tokenId is invalid', () => {
				// Arrange:
				const req = { params: { tokenId: 'AB12345' } };

				// Act + Assert:
				expect(() => mockServer.callRoute(route, req)).to.throw('tokenId has an invalid format');
			});

			it('throws error if there is no tokenId when sorting by balance', () => {
				// Arrange:
				const req = { params: { orderBy: 'balance' } };

				// Act + Assert:
				expect(() => mockServer.callRoute(route, req)).to.throw('tokenId must be provided when sorting by balance');
			});
		});
