			...
			describe('paginated', () => {
				...
				describe('parses filters', () => {
					...
					const testCases = [
						...
						{ filter: 'transferTokenId', param: '0000000000001000', value: [4096, 0] }
					];
					...
					it('fromTransferAmount', () => {
						const req = {
							params: {
								...
								transferTokenId: '0000000000001000',
						...
						return mockServer.callRoute(route, req).then(() => {
							...
							expect(dbTransactionsFake.firstCall.args[1].transferTokenId).to.deep.equal([4096, 0]);
						});
					});
				...
				describe('does not allow filtering by transfer amount if transfer token id is not provided', () => {
					const errorMessage = 'can\'t filter by transfer amount if `transferTokenId` is not provided';

					it('does not allow filtering by fromTransferAmount if transferTokenId is not provided', () => {
						const req = {
							params: {
								group: TransactionGroups.confirmed,
								fromTransferAmount: '12345'
					...
					it('does not allow filtering by toTransferAmount if transferTokenId is not provided', () => {
						const req = {
							params: {
								group: TransactionGroups.confirmed,
								toTransferAmount: '12345'
