...
describe('MerkleTree', () => {
	describe('merkle tree parse', () => {
		...
		it('getBitsFromMask', () => {
			let bits = merkleTree.getBitsFromMask(bitxorcore.utils.convert.hexToUint8('0C46'));
			expect(bits.length).to.equal(5);
			expect(bits.join('')).to.equal('239AE');

			bits = merkleTree.getBitsFromMask(bitxorcore.utils.convert.hexToUint8('8062'));
			expect(bits.length).to.equal(4);
			expect(bits.join('')).to.equal('79DE');

			bits = merkleTree.getBitsFromMask(bitxorcore.utils.convert.hexToUint8('1002'));
		...
		it('nibbleAt', () => {
			const path = bitxorcore.utils.convert.hexToUint8('B7BB382C56');
		...
		it('encodePath', () => {
			let path = bitxorcore.utils.convert.hexToUint8('B7BB382C56');
			expect(bitxorcore.utils.convert.uint8ToHex(merkleTree.encodePath(path, 10, false))).to.equal('00B7BB382C56');
			path = bitxorcore.utils.convert.hexToUint8('3DC610D300');
			expect(bitxorcore.utils.convert.uint8ToHex(merkleTree.encodePath(path, 9, false))).to.equal('13DC610D30');
			path = bitxorcore.utils.convert.hexToUint8('02396A7E61AE1B1C4C66BD6850C2951C77044CFA5BD0');
			expect(bitxorcore.utils.convert.uint8ToHex(merkleTree.encodePath(path, 43, true)))
		...
		it('parseBranch', () => {
			...
			merkleTree.parseBranch(bitxorcore.utils.convert.hexToUint8(branch), new Uint8Array(), 0);
		...
		it('parseLeaf', () => {
			...
			merkleTree = new MerkleTree();
			const { tree } = merkleTree;
			merkleTree.parseLeaf(bitxorcore.utils.convert.hexToUint8(leaf),
				bitxorcore.utils.convert.hexToUint8('04A7F2A487B42EA89323C4408F82415223ACFEC7DFA7924EFC31A70778AB17A0'), 63);
			expect(tree.length).to.equal(1);
		...
		it('parseRaw', () => {
			...
			merkleTree = new MerkleTree();
			const { tree } = merkleTree;
			merkleTree.parseMerkleTreeFromRaw(bitxorcore.utils.convert.hexToUint8(raw));
		...
		describe('merkle tree parse from testnet examples', () => {
			...
			trees.forEach(tree => {
				...
				it(`${fieldName} ${tree[fieldName]}`, () => {
					...
					testnetTree.parseMerkleTreeFromRaw(bitxorcore.utils.convert.hexToUint8(tree.raw));
