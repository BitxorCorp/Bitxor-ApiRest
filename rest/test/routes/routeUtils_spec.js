...
describe('route utils', () => {
	describe('parse argument', () => {
		...
		describe('address', () => addParserTests({
			parser: 'address',
			valid: addresses.valid.map(id => ({ id, parsed: bitxorcore.model.address.stringToAddress(id) })),
			invalid: [
		...
		describe('publicKey', () => addParserTests({
			parser: 'publicKey',
			valid: publicKeys.valid.map(id => ({ id, parsed: bitxorcore.utils.convert.hexToUint8(id) })),
			invalid: [
		...
		describe('accountId', () => {
			describe('address', () => addParserTests({
				parser: 'accountId',
				valid: addresses.valid.map(id => ({ id, parsed: ['address', bitxorcore.model.address.stringToAddress(id)] })),
				invalid: [
					{ id: addresses.invalid, error: 'illegal base32 character 1' }
				]
			}));
			...
			describe('publicKey', () => addParserTests({
				parser: 'accountId',
				valid: publicKeys.valid.map(id => ({ id, parsed: ['publicKey', bitxorcore.utils.convert.hexToUint8(id)] })),
				invalid: [
					{ id: publicKeys.invalid, error: 'unrecognized hex char \'1G\'' }
				]
			}));
		...
		describe('hash256', () => addParserTests({
			parser: 'hash256',
			valid: hashes256.valid.map(hash => ({ id: hash, parsed: bitxorcore.utils.convert.hexToUint8(hash) })),
			invalid: hashes256.invalid.map(hash => ({ id: hash, error: `invalid length of hash256 '${hash.length}` }))
		}));

		describe('hash512', () => addParserTests({
			parser: 'hash512',
			valid: hashes512.valid.map(hash => ({ id: hash, parsed: bitxorcore.utils.convert.hexToUint8(hash) })),
	...
	describe('addPutPacketRoute', () => {
		const registrar = (server, db, services) => {
			const parseHexParam = (params, key) => routeUtils.parseArgument(params, key, bitxorcore.utils.convert.hexToUint8);
		...
		it('returns a merkle path for valid height and hash', () => {
			...
			return processorFunction(req, { send: sendFake }, nextFake).then(() => {
				...
				expect(sendFake.calledOnceWith(sinon.match({
					payload: {
						merklePath: [
							{ position: bitxorcore.crypto.merkle.NodePosition.right, hash: merkleTree[3].buffer },
							{ position: bitxorcore.crypto.merkle.NodePosition.left, hash: merkleTree[4].buffer }
						]
					},
	...
	describe('addressToPublicKey', () => {
		...
		const accountAddress = bitxorcore.model.address.stringToAddress(addresses.valid[0]);
