...
const { address, EntityType } = bitxorcore.model;
...
describe('bitxorcore db', () => {
	...
	const runDbTest = (dbEntities, issueDbCommand, assertDbCommandResult) => {
		// Arrange:
		const db = new BitxorcoreDb(Object.assign({ networkId: Testnet_Network }, DefaultPagingOptions));
	...
	describe('basic', () => {
		it('cannot create db without network id', () => {
			// Act + Assert:
			expect(() => new BitxorcoreDb({})).to.throw('network id is required');
		});

		it('can close unconnected db', () => {
			// Arrange:
			const db = new BitxorcoreDb({ networkId: Testnet_Network });
	...
	const createTransactionHash = id => bitxorcore.utils.convert.hexToUint8(`${'00'.repeat(16)}${id.toString(16)}`.slice(-32));
	...
	describe('transaction by id', () => {
		...
		describe('translates group to collection name', () => {
			...
			const runTransactionsByIdTest = (dbCall, param, group, collection) => {
				it(group, () => {
					// Arrange:
					const transactionsByIdsImplStub = sinon.stub(BitxorcoreDb.prototype, 'transactionsByIdsImpl').returns('');
					const db = new BitxorcoreDb(Object.assign({ networkId: Testnet_Network }, DefaultPagingOptions));
	...
	describe('transactions', () => {
		...
		const createTransaction = (objectId, addresses, height, signerPublicKey, recipientAddress, type, tokens) => ({
			_id: createObjectId(objectId),
			meta: {
				height: Long.fromNumber(height),
		...
		it('ignores inner aggregate transactions in the results', () => {
			...
			const dbTransactions = [
				...
				createInnerTransaction(100, 30, 0, 0, EntityType.tokenDefinition),
				createInnerTransaction(200, 30, 0, 0, EntityType.tokenSupplyChange),

				createTransaction(20, [], 1, 0, 0, EntityType.aggregateBonded),
				createInnerTransaction(300, 30, 0, 0, EntityType.transfer),
				createInnerTransaction(400, 30, 0, 0, EntityType.tokenDefinition),
			...
			const filters = {
				transactionTypes: [EntityType.transfer, EntityType.tokenDefinition, EntityType.aggregateComplete]
			};
		...
		describe('correctly applies each filter:', () => {
			...
			it('transactionTypes', () => {
				...
				const filters = {
					transactionTypes: [EntityType.tokenDefinition, EntityType.aggregateComplete, EntityType.transfer]
				};
			...
			it('transferTokenId', () => {
				...
				const filters = { transferTokenId: 20 };
			...
			it('fromTransferAmount', () => {
				...
				const filters = { transferTokenId: 10, fromTransferAmount: 6 };
			...
			it('toTransferAmount', () => {
				...
				const filters = { transferTokenId: 10, toTransferAmount: 6 };
	...
	describe('accounts', () => {
		...
		const tokenIdTest1 = Long.fromNumber(12345678);
		const tokenIdTest2 = Long.fromNumber(87654321);
		...
		const createAccount = (objectId, accountAddress, tokens, importances) => ({
			_id: createObjectId(objectId),
			account: {
		...
		const runTestAndVerifyIds = (dbAccounts, accountAddress, tokenId, options, expectedIds) => {
			const expectedObjectIds = expectedIds.map(id => createObjectId(id));

			return runDbTest(
				{ accounts: dbAccounts },
				db => db.accounts(accountAddress, tokenId, options),
				page => {
		...
		const runAccountsFilteredByAddressTest = pagination => {
			it('returns accounts filtered by accountAddress', () => {
				// Arrange:
				const dbAccounts = [
					createAccount(10, addressTest1, [{ id: tokenIdTest1, amount: Long.fromNumber(1) }]),
					createAccount(20, addressTest2, [{ id: tokenIdTest1, amount: Long.fromNumber(1) }]),
		...
		const runAccountsFilteredByTokenIdTest = pagination => {
			it('returns accounts filtered by tokenId', () => {
				// Arrange:
				const dbAccounts = [
					createAccount(10, addressTest1, [{ id: tokenIdTest1, amount: Long.fromNumber(1) }]),
					createAccount(20, addressTest1, [{ id: tokenIdTest2, amount: Long.fromNumber(1) }])
				];

				// Act + Assert:
				return runTestAndVerifyIds(dbAccounts, undefined, tokenIdTest2, pagination, [20]);
			});
		};
		...
		it('returns empty for unknown tokenId', () => {
			// Arrange:
			const dbAccounts = [
				createAccount(10, addressTest1, [
					{ id: tokenIdTest1, amount: Long.fromNumber(1) }
				])
			];

			// Act + Assert:
			return runTestAndVerifyIds(dbAccounts, undefined, tokenIdTest2, paginationOptions, []);
		});

		runAccountsFilteredByAddressTest(paginationOptions);

		runAccountsFilteredByTokenIdTest(paginationOptions);

		it('returns all accounts if no accountAddress or tokenId are provided', () => {
			// Arrange:
			const dbAccounts = [
				createAccount(10, addressTest1, [{ id: tokenIdTest1, amount: Long.fromNumber(1) }]),
				createAccount(20, addressTest2, [{ id: tokenIdTest2, amount: Long.fromNumber(1) }]),
				createAccount(30, addressTest3, [{ id: tokenIdTest1, amount: Long.fromNumber(1) }]),
				createAccount(40, addressTest4, [{ id: tokenIdTest2, amount: Long.fromNumber(1) }])
			];
		...
		describe('respects offset', () => {
			...
			const dbAccounts = () => ([
				createAccount(10, addressTest1, [{ id: tokenIdTest1, amount: Long.fromNumber(1) }]),
				createAccount(20, addressTest2, [{ id: tokenIdTest1, amount: Long.fromNumber(2) }]),
				createAccount(30, addressTest3, [{ id: tokenIdTest1, amount: Long.fromNumber(3) }])
			]);
			const options = {
				pageSize: 10,
				pageNumber: 1,
		...
		describe('respects sort conditions', () => {
			...
			const dbAccounts = () => ([
				createAccount(10, addressTest1, [{ id: tokenIdTest1, amount: Long.fromNumber(1) }]),
				createAccount(20, addressTest1, [{ id: tokenIdTest2, amount: Long.fromNumber(1) }]),
				createAccount(30, addressTest2, [{ id: tokenIdTest1, amount: Long.fromNumber(1) }])
			]);
		...
		describe('when sorting by balance', () => {
			...
			it('returns expected structure', () => {
				// Arrange:
				const dbAccounts = [createAccount(10, addressTest1, [{ id: tokenIdTest1, amount: Long.fromNumber(1) }])];
			...
			it('returns empty for unknown tokenId', () => {
				...
				// Act + Assert:
				return runTestAndVerifyIds(dbAccounts, undefined, tokenIdTest2, paginationOptionsBalanceSorting, []);
			});

			runAccountsFilteredByAddressTest(paginationOptionsBalanceSorting);

			runAccountsFilteredByTokenIdTest(paginationOptionsBalanceSorting);
			...
			describe('picks top importance', () => {
				...
				it('one importance', () => {
					...
					const dbAccounts = [createAccount(10, addressTest1, [{ id: tokenIdTest1, amount: Long.fromNumber(1) }], importances)];
			...
			describe('respects offset', () => {
				...
				it('gt', () => {
					...
					// Act + Assert:
					return runTestAndVerifyIds(dbAccounts(), undefined, tokenIdTest1, options, [30]);
				});
				...
				it('lt', () => {
					...
					// Act + Assert:
					return runTestAndVerifyIds(dbAccounts(), undefined, tokenIdTest1, options, [10]);
				});
			});

			describe('sorts by correct token', () => {
				const seedAccounts = [];
				...
				const tokenAmountEquals = (accountsPage, accountIndex, tokenId, amount) =>
					accountsPage.data[accountIndex].account.tokens
						.find(token => token.id.equals(Long.fromNumber(tokenId))).amount.equals(Long.fromNumber(amount));
				...
				it('asc', () =>
					runDbTest(
						...
						accountsPage => {
							expect(tokenAmountEquals(accountsPage, 0, 33, 2)).to.equal(true);
							expect(tokenAmountEquals(accountsPage, 1, 33, 5)).to.equal(true);
							expect(tokenAmountEquals(accountsPage, 2, 33, 8)).to.equal(true);
						}
					));
				...
				it('desc', () =>
					runDbTest(
						...
						accountsPage => {
							expect(tokenAmountEquals(accountsPage, 0, 33, 8)).to.equal(true);
							expect(tokenAmountEquals(accountsPage, 1, 33, 5)).to.equal(true);
							expect(tokenAmountEquals(accountsPage, 2, 33, 2)).to.equal(true);
						}
					));
			});
		});
	...
	describe('accounts by ids', () => {
		...
		const addAccountsByIdsTestsForSingleAccountLookup = (knownAccountId, unknownAccountId) => {
			runSingleKnownAccountTest(
				'can retrieve account with neither public key nor tokens',
				knownAccountId,
				{ savePublicKey: false, numTokens: 0 }
			);
			runSingleKnownAccountTest(
				'can retrieve account with public key but no tokens',
				knownAccountId,
				{ savePublicKey: true, numTokens: 0 }
			);
			runSingleKnownAccountTest(
				'can retrieve account without public key but with tokens',
				knownAccountId,
				{ savePublicKey: false, numTokens: 3 }
			);
			runSingleKnownAccountTest(
				'can retrieve account with public key and with tokens',
				knownAccountId,
				{ savePublicKey: true, numTokens: 3 }
			);
			runSingleKnownAccountTest(
				'can retrieve account without importances',
				knownAccountId,
				{ savePublicKey: true, numTokens: 3, numImportances: 0 }
			);
			runSingleKnownAccountTest(
				'can retrieve account with single importance',
				knownAccountId,
				{ savePublicKey: true, numTokens: 3, numImportances: 1 }
			);
			runUnknownAccountTest(
				unknownAccountId,
				{ savePublicKey: false, numTokens: 3, expectedAddress: decodedAddressUnknown }
			);
		};
		...
		describe('account from decoded address', () => {
			...
			it('picks top importance', () => {
				...
				const seedAccounts = test.db.createAccounts(publicKey, { savePublicKey: true, numTokens: 1, numImportances });
