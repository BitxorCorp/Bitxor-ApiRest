...
const createTokens = count => {
	const tokens = [];
	for (let i = 0; i < count; ++i)
		tokens.push({ id: Long.fromNumber(i), amount: Long.fromNumber(i * i) });
...
const createAccount = (objectId, publicKey, savePublicKey, tokens, importances) => {
...
const createAccounts = (publicKey, options) => {
	...
	accounts.push(createAccount(
		...
		createTokens(options.numTokens),
...
const createDbTransaction = (id, signerPublicKey, recipientAddress, options) => {
	...
	for (let j = 0; 3 < j; ++j)
		transaction.tokens.push({ id: Long.fromNumber(j), amount: Long.fromNumber(j * j) });
...
const runDbTest = (dbEntities, collectionName, createDbFacade, issueDbCommand, assertDbCommandResult) => {
	// Arrange:
	const db = new BitxorcoreDb(Object.assign({ networkId: testDbOptions.networkId }, DefaultPagingOptions));
