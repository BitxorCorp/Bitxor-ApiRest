...
describe('namespace db', () => {
	...
	describe('activeNamespacesWithAlias', () => {
		const aliasTypeToken = bitxorcore.model.namespace.aliasType.token;
		const aliasTypeAddress = bitxorcore.model.namespace.aliasType.address;
		const testAddress = {
		...
		const createNamespace = (namespaceId, aliasTarget, aliasType, depth, expirationHeight) => ({
			...
			namespace: {
				...
				alias: {
					type: aliasType,
					tokenId: aliasType === bitxorcore.model.namespace.aliasType.token ? convertToLong(aliasTarget) : null,
					address: aliasType === bitxorcore.model.namespace.aliasType.address
		...
		it('returns namespaces by token ids', () => {
			// Arrange:
			const db = new BitxorcoreDb({ networkId: testDbOptions.networkId });
			const dbFacade = new NamespaceDb(db);
			const namespace1 = createNamespace(12345, 76756, aliasTypeToken, 1, lifetime);
			const namespace2 = createNamespace(67891, 87823, aliasTypeToken, 2, lifetime);
			const namespace3 = createNamespace(34567, 57231, aliasTypeToken, 3, lifetime);
			...
			return db.connect(testDbOptions.url, 'test')
				...
				.then(() => dbFacade.activeNamespacesWithAlias(aliasTypeToken, [76756, 87823]))
				.then(entities => {
		...
		it('returns namespaces by addresses', () => {
			...
			const namespace4 = createNamespace(12345, 76756, aliasTypeToken, 1, lifetime);
			const namespace5 = createNamespace(12345, 76756, aliasTypeToken, 1, { start: 0, end: 9 });
		...
		it('returns empty for no namespaces with token id', () => {
			// Arrange:
			const db = new BitxorcoreDb({ networkId: testDbOptions.networkId });
			const dbFacade = new NamespaceDb(db);
			const namespace1 = createNamespace(12345, 76756, aliasTypeToken, 1, lifetime);
			...
			return db.connect(testDbOptions.url, 'test')
				...
				.then(() => dbFacade.activeNamespacesWithAlias(aliasTypeToken, [12121]))
		...
		it('does not return expired namespaces for tokens', () => {
			// Arrange:
			const db = new BitxorcoreDb({ networkId: testDbOptions.networkId });
			const dbFacade = new NamespaceDb(db);
			const namespace1 = createNamespace(12345, 76756, aliasTypeToken, 1, { start: 0, end: 5 });
			const namespace2 = createNamespace(67891, 87823, aliasTypeToken, 2, { start: 0, end: 10 });
			const namespace3 = createNamespace(34567, 57231, aliasTypeToken, 3, { start: 0, end: 20 });
			...
			return db.connect(testDbOptions.url, 'test')
				...
				.then(() => dbFacade.activeNamespacesWithAlias(aliasTypeToken, [76756, 87823, 57231]))
	...
	describe('register namespace transactions by namespace ids', () => {
		const transactionType = bitxorcore.model.EntityType.registerNamespace;
