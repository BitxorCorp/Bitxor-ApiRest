			...
			return test.route.executeSingle(
				...
				response => {
					...
					dbParamTuples.forEach(dbParamTuple => {
						...
						expect(dbParamTuple.transactionType).to.deep.equal(bitxorcore.model.EntityType.registerNamespace);
	...
	describe('get token names', () => {
		describe('calls aliasNamesRoutesProcessor with correct params', () => {
			it('is called once for each endpoint using it with correct parameters', () => {
				...
				expect(aliasNamesRoutesProcessorSpy.firstCall.args[1]).to.equal(bitxorcore.model.namespace.aliasType.token);
				expect(aliasNamesRoutesProcessorSpy.firstCall.args[4]).to.equal('tokenId');
				expect(aliasNamesRoutesProcessorSpy.firstCall.args[5]).to.equal('tokenNames');

				aliasNamesRoutesProcessorSpy.restore();
			});

			describe('getParams parses token ids correctly', () => {
				const aliasNamesRoutesProcessorSpy = sinon.spy(namespaceUtils, 'aliasNamesRoutesProcessor');
				const routes = {};
				const server = {
					get: (path, handler) => { routes[path] = handler; },
				...
				it('parses token ids correctly', () => {
					// Arrange:
					const req = { params: { tokenIds: ['78A4895CB6653DE4', '56AB67FF45468988'] } };
				...
				it('parses empty token ids list correctly', () => {
					// Arrange:
					const req = { params: { tokenIds: [] } };

					// Act + Assert:
					expect(getParams(req)).to.deep.equal([]);
				});

				it('returns 409 if provided token id is invalid', () => {
					// Arrange:
					const req = { params: { tokenIds: ['78A4895CB6653DE4', '123XXX', '56AB67FF45468988'] } };

					// Act + Assert:
					expect(() => { getParams(req); })
						.to.throw('element in array tokenIds has an invalid format');
				});
			});
			...
			describe('namespaceFilter filters namespaces correctly', () => {
				...
				it('filters namespaces aliasing tokenId correctly', () => {
					// Arrange:
					const id = 12345;
					const namespace1 = {
						namespace: { alias: { tokenId: convertToLong(id) } }
					};
	...
	describe('get account names', () => {
		describe('calls aliasNamesRoutesProcessor with correct params', () => {
			...
			it('is called once for each endpoint using it with correct parameters', () => {
				...
				// Assert:
				expect(aliasNamesRoutesProcessorSpy.calledTwice).to.equal(true);
				expect(aliasNamesRoutesProcessorSpy.secondCall.args[1]).to.equal(bitxorcore.model.namespace.aliasType.address);
