...
const supplyRoutes = require('../../../src/plugins/token/supplyRoutes');
...
describe('supply routes', () => {
	describe('network currency supply', () => {
		...
		const currencyTokenId = '0x1234\'5678\'ABCD\'EF01';
		const genesisSignerPublicKey = 'AF1B9DCF4FAD2CDC2C04B4F9CBDF3C9C884A9F05B40A59E233681E282DC824D9';
		...
		const createAccountSample = (publicKey, currencyAmount, otherAmount) => ({
			address: '',
			addressHeight: '',
			publicKey: bitxorcore.utils.convert.hexToUint8(publicKey),
			publicKeyHeight: '',
		...
		const accountsSample = [
			{ id: 'random1', account: createAccountSample(genesisSignerPublicKey, 1000000, 9000000) },
		...
		const dbTokensFake = sinon.fake(() => Promise.resolve(tokensSample));
		const dbAccountsFake = sinon.fake(accountIds => {
			const filteredAccountsSample = accountsSample.filter(accountSample =>
				accountIds.some(accountId => bitxorcore.utils.array.deepEqual(accountId.publicKey, accountSample.account.publicKey)));
			...
			it('network currency supply circulating (without burns)', () => {
				sinon.stub(fs, 'readFile').callsFake((path, data, callback) =>
					callback(null, [
						...
						'',
						'[chain]',
						'currencyTokenId = 0x1234\'5678\'ABCD\'EF02'
					].join('\n')));
