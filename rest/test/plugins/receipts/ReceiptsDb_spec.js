...
describe('receipts db', () => {
	...
	describe('transactionStatements', () => {
		...
		const createReceipt = fields => ({
			...
			tokenId: fields.tokenId ? convertToLong(fields.tokenId) : undefined
		});
		...
		describe('all the provided filters are taken into account', () => {
			...
			describe('artifactId', () => {
				...
				// TokenIds examples: 1D9CDC7E218CA88D, 24F426B8D5493D4B, 49F6C0F0163730A9

				const namespaceId1 = uint64.fromHex('941299B2B7E1291C');
				const namespaceId2 = uint64.fromHex('85BBEA6CC462B244');
				const tokenId1 = uint64.fromHex('1D9CDC7E218CA88D');
				const tokenId2 = uint64.fromHex('24F426B8D5493D4B');

				it('namespaceId', () => {
					// Arrange:
					const dbReceipts = [
						createTransactionStatement(10, 100, [createReceipt({ namespaceId: namespaceId1, tokenId: namespaceId2 })]),
						createTransactionStatement(20, 110, [
				...
				it('tokenId', () => {
					// Arrange:
					const dbReceipts = [
						createTransactionStatement(10, 100, [createReceipt({ namespaceId: tokenId2, tokenId: tokenId1 })]),
						createTransactionStatement(20, 110, [
							createReceipt({ tokenId: tokenId1 }),
							createReceipt({ tokenId: tokenId2 })
						]),
						createTransactionStatement(30, 120, [createReceipt({ tokenId: tokenId2 })]),
						createTransactionStatement(40, 130, [])
					];
					const filters = { artifactId: tokenId2 };
