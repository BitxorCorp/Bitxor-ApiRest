...
describe('receipts routes', () => {
	...
	describe('artifact resolution statements', () => {
		...
		describe('GET', () => {
			...
			it('returns empty token statements page if no statements found', () => {
				// Arrange:
				const req = { params: { artifact: 'token', height: '666' } };
			...
			it('forwards token artifact', () => {
				// Arrange:
				const req = { params: { artifact: 'token' } };

				// Act:
				return mockServer.callRoute(route, req).then(() => {
					// Assert:
					expect(dbArtifactStatementsFake.calledOnce).to.equal(true);
					expect(dbArtifactStatementsFake.firstCall.args[1]).to.equal('token');
