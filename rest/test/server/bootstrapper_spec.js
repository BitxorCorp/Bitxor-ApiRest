	...
	[(options && options.formatterName) || 'json']: {
		...
		blockHeaderWithMetadata: {
			...
			format: blockHeaderWithMetadata => {
				...
				return {
					height: block.height,
					signerPublicKey: bitxorcore.utils.convert.uint8ToHex(block.signerPublicKey)
				};
			}
		}
	}
...
describe('server (bootstrapper)', () => {
	...
	describe('HTTP', () => {
		...
		const addCommonTestsForHttpMethod = method => {
			...
			it('omits CORS response if no config provided', done => {
				...
				const request = {
					method: 'GET',
					headers: { origin: 'http://bitxor.example' }
				};
			...
			it('builds CORS response with matching origin in the provided config', done => {
				// Arrange:
				const crossDomainAdder = bootstrapper.createCrossDomainHeaderAdder({
					allowedMethods: ['GET'], allowedHosts: ['http://bitxor.example']
				});
				const request = {
					method: 'GET',
					headers: { origin: 'http://bitxor.example' }
				...
				expect(response.header.calledWith('Access-Control-Allow-Origin', 'http://bitxor.example')).to.equal(true);
		...
		describe('OPTIONS', () => {
			const makeJsonHippieForOptions = route => {
				...
				return hippie(server)
					.header('Origin', 'http://bitxor.example')
					.url(route)
					.method('OPTIONS')
					.json()
					.form();
	...
	describe('websockets', () => {
		...
		const registerRoute = (server, route) => {
			...
			const modelSystem = bitxorcore.plugins.bitxorcoreModelSystem.configure([], {});
			const config = {
