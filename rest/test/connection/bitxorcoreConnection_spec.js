...
const bitxorcoreConnection = require('../../src/connection/bitxorcoreConnection');
const { expect } = require('chai');

describe('bitxorcore connection', () => {
	...
	describe('send', () => {
		const assertSend = (testName, assertCallback) => {
			it(testName, () => {
				...
				// Act:
				const promise = bitxorcoreConnection.wrap(context.mockConnection).send(payload);
	...
	describe('pushPull', () => {
		const assertPushPull = (testName, assertCallback) => {
			it(testName, () => {
				...
				// Act:
				const promise = bitxorcoreConnection.wrap(context.mockConnection).pushPull(payload, 1000);
