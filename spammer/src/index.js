...
const { serialize, transactionExtensions } = bitxorcore.modelBinary;
...
(() => {
	const Testnet_Network = bitxorcore.model.networkInfo.networks.testnet.id;
	...
	const predefinedRecipient = (() => {
		...
		for (let i = 0; i < options.predefinedRecipients; ++i) {
			const keyPair = bitxorcore.crypto.createKeyPairFromPrivateKeyString(curPrivateKey);
			curPrivateKey = bitxorcore.utils.convert.uint8ToHex(keyPair.publicKey);
	...
	const randomRecipient = () => {
		...
		const keyPair = bitxorcore.crypto.createKeyPairFromPrivateKeyString(bitxorcore.utils.convert.uint8ToHex(privateKey));
		return address.publicKeyToAddress(keyPair.publicKey, Testnet_Network);
	};

	const pickKeyPair = (privateKeys => {
		const keyPairs = privateKeys.map(privateKey => bitxorcore.crypto.createKeyPairFromPrivateKeyString(privateKey));
	...
	const randomKeyPair = () => {
		const keySize = 32;
		const privateKey = crypto.randomBytes(keySize);
		return bitxorcore.crypto.createKeyPairFromPrivateKeyString(bitxorcore.utils.convert.uint8ToHex(privateKey));
	};
	...
	const createTransfer = (signerPublicKey, recipientAddress, transferId, amount) => {
		...
		transfer.tokens[0].amount = amount;
		return transfer;
	};
