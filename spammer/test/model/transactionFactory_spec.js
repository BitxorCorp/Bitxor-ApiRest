...
const Signature_Size = bitxorcore.constants.sizes.signature;
const Address_Decoded_Size = bitxorcore.constants.sizes.addressDecoded;
...
describe('transaction factory', () => {
	...
	const assertTransactionData = (transaction, keyPair, version, type) => {
		const txDeadline = bitxorcore.utils.uint64.compact(transaction.deadline);
	...
	describe('random transfer', () => {
		it('can be created with reasonable data', () => {
			...
			expect(transaction.tokens[0].id).to.deep.equal([0xD95FCF29, 0xD525AD41]);
		});
	});
