	...
	describe('register schema', () => {
		it('adds lock hash system schema', () => {
			...
			assertSchema(modelSchema['hashLockInfo.lock'], 7,
				'version', 'ownerAddress', 'tokenId', 'amount', 'endHeight', 'status', 'hash');

			// - hash lock transaction
			const transactionSchemaSize = Object.keys(modelSchema.transaction).length;
			assertSchema(modelSchema.hashLock, transactionSchemaSize + 4, 'duration', 'hash', 'tokenId', 'amount');
		});
	});
		...
		describe('supports hash lock', () => {
			...
			test.binary.test.addAll(getCodec(EntityType.hashLock), 56, () => ({
				buffer: Buffer.concat([
					Buffer.of(0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF), // tokenId
