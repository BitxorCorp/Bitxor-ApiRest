	...
	describe('register schema', () => {
		it('adds namespace system schema', () => {
			...
			expect(modelSchema).to.contain.all.keys(
				...
				'namespaceDescriptor.alias.token',
			...
			// - alias token
			expect(Object.keys(modelSchema.aliasToken).length).to.equal(Object.keys(modelSchema.transaction).length + 3);
			expect(modelSchema.aliasToken).to.contain.all.keys(['namespaceId', 'tokenId', 'aliasAction']);
			...
			// - namespaceDescriptor.alias.token
			expect(Object.keys(modelSchema['namespaceDescriptor.alias.token']).length).to.equal(2);
			expect(modelSchema['namespaceDescriptor.alias.token']).to.contain.all.keys(['type', 'tokenId']);
			...
			// - token names
			expect(Object.keys(modelSchema.tokenNames).length).to.equal(1);
			expect(modelSchema.tokenNames).to.contain.all.keys(['tokenNames']);

			// - token names tuple
			expect(Object.keys(modelSchema.tokenNamesTuple).length).to.equal(2);
			expect(modelSchema.tokenNamesTuple).to.contain.all.keys(['tokenId', 'names']);
	...
	describe('conditional schema', () => {
		describe('uses the correct conditional schema depending on alias type', () => {
			...
			it('formats alias token type', () => {
				...
				// Act:
				const formattedAlias = formatAlias(aliasToken);

				// Assert:
				expect(formattedAlias).to.contain.all.keys(['type', 'tokenId']);
				expect(formattedAlias).deep.equal({
					type: 'uint8',
					tokenId: 'uint64HexIdentifier'
				});
			});
	...
	describe('register codecs', () => {
		...
		it('adds namespace codecs', () => {
			...
			expect(codecs).to.contain.all.keys([
				...
				EntityType.aliasToken.toString(),
		...
		describe('supports alias token', () => {
			test.binary.test.addAll(getCodec(EntityType.aliasToken), constants.sizes.aliasToken, () => ({
				buffer: Buffer.concat([
					Buffer.of(0xF2, 0x26, 0x6C, 0x06, 0x40, 0x83, 0xB2, 0x92), // namespace id
					Buffer.of(0xCA, 0xD0, 0x8E, 0x6E, 0xFF, 0x21, 0x2F, 0x49), // token id
					Buffer.of(0xCA) // alias action
				]),

				object: {
					namespaceId: [0x066C26F2, 0x92B28340],
					tokenId: [0x6E8ED0CA, 0x492F21FF],
					aliasAction: 0xCA
				}
			}));
		});
