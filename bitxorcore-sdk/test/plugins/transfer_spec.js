...
const constants = {
	...
	offsets: {
		messageSize: 2 + 1 + 4 + 1,		// messageSize + tokensCount + reserved1 + reserved2
		tokensCount: 1 + 4 + 1	// tokensCount + reserved1 + reserved2
	}
};
...
describe('transfer plugin', () => {
	describe('register schema', () => {
		it('adds transfer system schema', () => {
			...
			expect(modelSchema.transfer).to.contain.all.keys(['recipientAddress', 'message', 'tokens']);
		});
	});
	...
	describe('register codecs', () => {
		...
		const addTokens = generator => {
			const Tokens_Buffer = Buffer.from([
			...
			return () => {
				...
				data.buffer.writeUInt8(5, constants.sizes.transfer - constants.offsets.tokensCount);
		...
		describe('supports transfer', () => {
			describe('with neither message nor tokens', () => {
			...
			describe('with tokens', () => {
				test.binary.test.addAll(getCodec(), constants.sizes.transfer + constants.sizes.tokens, addTokens(generateTransaction));
			});

			describe('with message and tokens', () => {
				test.binary.test.addAll(
					getCodec(),
					constants.sizes.transfer + constants.sizes.message + constants.sizes.tokens,
					addMessage(addTokens(generateTransaction))
				);
			});
		});
	});
