	...
	describe('register schema', () => {
		it('adds lock secret system schema', () => {
			...
			assertSchema(modelSchema['secretLockInfo.lock'], 10,
				'version', 'ownerAddress', 'tokenId', 'amount', 'endHeight', 'secret',
			...
			assertSchema(modelSchema.secretLock, transactionSchemaSize + 6,
				'secret', 'tokenId', 'amount', 'duration', 'recipientAddress', 'hashAlgorithm');
		...
		describe('supports secret lock', () => {
			...
			test.binary.test.addAll(getCodec(EntityType.secretLock), 81, () => ({
				buffer: Buffer.concat([
					...
					Buffer.of(0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF), // token
