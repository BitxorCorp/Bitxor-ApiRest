...
describe('restrictions plugin', () => {
	describe('account restriction types enumeration', () => {
		it('contains valid values', () => {
			...
			expect(AccountRestrictionType.tokenAllow).to.equal(0x0002);
			expect(AccountRestrictionType.tokenBlock).to.equal(0x0002 + accountRestrictionTypeBlockOffset);
	...
	describe('register schema', () => {
		...
		const accountRestrictionSchemas = [
			...
			'accountRestriction.tokenAccountRestriction',
		...
		it('adds restrictions system schema', () => {
			...
			expect(modelSchema).to.contain.all.keys([
				...
				'accountRestrictionToken',
				'accountRestrictionOperation',
				'accountRestrictions',
				'accountRestriction.restrictions',
				'accountRestriction.fallback',
				'tokenRestrictions',
				'tokenRestrictions.entry',
				'tokenRestrictions.entry.restrictions',
				'tokenRestrictions.entry.restrictions.restriction'
		...
		it('adds account restrictions schemas', () => {
			...
			// - accountRestrictionToken
			expect(Object.keys(modelSchema.accountRestrictionToken).length).to.equal(Object.keys(modelSchema.transaction).length + 3);
			expect(modelSchema.accountRestrictionToken).to.contain.all.keys([
			...
			// - accountRestriction address, token, and operation restrictions
		...
		it('uses the correct conditional account schema depending on restriction type', () => {
			...
			expect(accountRestrictionSchema({ restrictionFlags: AccountRestrictionType.addressBlock }))
				.to.equal('accountRestriction.addressAccountRestriction');
			expect(accountRestrictionSchema({ restrictionFlags: AccountRestrictionType.tokenAllow }))
				.to.equal('accountRestriction.tokenAccountRestriction');
			expect(accountRestrictionSchema({ restrictionFlags: AccountRestrictionType.tokenBlock }))
				.to.equal('accountRestriction.tokenAccountRestriction');
		...
		it('adds token restrictions system schemas', () => {
			// - token restriction address transaction
			expect(Object.keys(modelSchema.tokenRestrictionAddress).length).to.equal(Object.keys(modelSchema.transaction).length + 5);
			expect(modelSchema.tokenRestrictionAddress).to.contain.all.keys([
				'tokenId',
				'restrictionKey',
				'targetAddress',
			...
			// - token restriction global transaction
			expect(Object.keys(modelSchema.tokenRestrictionGlobal).length).to.equal(Object.keys(modelSchema.transaction).length + 7);
			expect(modelSchema.tokenRestrictionGlobal).to.contain.all.keys([
				'tokenId',
				'referenceTokenId',
				'restrictionKey',
				'previousRestrictionValue',
			...
			// - token restrictions
			expect(Object.keys(modelSchema.tokenRestrictions).length).to.equal(2);
			expect(modelSchema.tokenRestrictions).to.contain.all.keys(['id', 'tokenRestrictionEntry']);

			// - tokenRestriction.entry
			expect(Object.keys(modelSchema['tokenRestrictions.entry']).length).to.equal(6);
			expect(modelSchema['tokenRestrictions.entry']).to.contain.all.keys([
				'version', 'compositeHash', 'entryType', 'tokenId', 'targetAddress', 'restrictions'
			]);

			// - tokenRestrictions.entry.restrictions
			expect(Object.keys(modelSchema['tokenRestrictions.entry.restrictions']).length).to.equal(3);
			expect(modelSchema['tokenRestrictions.entry.restrictions']).to.contain.all.keys([
				'key', 'value', 'restriction'
			]);

			// - tokenRestrictions.entry.restrictions.restriction
			expect(Object.keys(modelSchema['tokenRestrictions.entry.restrictions.restriction']).length).to.equal(3);
			expect(modelSchema['tokenRestrictions.entry.restrictions.restriction']).to.contain.all.keys([
				'referenceTokenId', 'restrictionValue', 'restrictionType'
			]);
		});
	});
	...
	describe('register codecs', () => {
		...
		it('adds restriction codecs (account and token)', () => {
			// Act:
			...
			expect(codecs).to.contain.all.keys([
				...
				EntityType.accountRestrictionToken.toString(),
				EntityType.accountRestrictionOperation.toString(),
				EntityType.tokenRestrictionAddress.toString(),
				EntityType.tokenRestrictionGlobal.toString()
			]);
		});

		describe('account restrictions codecs', () => {
			const addressCodec = getCodecs()[EntityType.accountRestrictionAddress];
			const tokenCodec = getCodecs()[EntityType.accountRestrictionToken];
			...
			describe('supports account token restriction with no additions or deletions', () => {
				addNoModificationTests(AccountRestrictionType.tokenAllow, tokenCodec);
			});
			...
			describe('supports account token restriction with one addition and deletion', () => {
				addModificationTests(
					AccountRestrictionType.tokenAllow,
					tokenCodec,
			...
			describe('supports account token restriction with several additions and deletions', () => {
				const token1 = Buffer.of(0xF2, 0x26, 0x6C, 0x06, 0x40, 0x83, 0xB2, 0x10);
				const token2 = Buffer.of(0xB2, 0x26, 0x6C, 0x06, 0x40, 0x83, 0xB2, 0x11);
				const token3 = Buffer.of(0xD2, 0x26, 0x6C, 0x06, 0x40, 0x83, 0xB2, 0x12);

				const token4 = Buffer.of(0xAA, 0x00, 0x4B, 0x33, 0x25, 0x38, 0x00, 0x13);
				const token5 = Buffer.of(0xBB, 0x00, 0x4B, 0x33, 0x25, 0x38, 0x00, 0x14);

				addModificationTests(
					AccountRestrictionType.tokenAllow,
					tokenCodec,
					Buffer.concat([token1, token2, token3]),
					[[0x066C26F2, 0x10B28340], [0x066C26B2, 0x11B28340], [0x066C26D2, 0x12B28340]],
					Buffer.concat([token4, token5]),
		...
		describe('token restrictions codecs', () => {
			const getCodec = entityType => getCodecs()[entityType];

			describe('supports token restriction address', () => {
				const targetAddress = test.random.bytes(test.constants.sizes.addressDecoded); // 24

				test.binary.test.addAll(getCodec(EntityType.tokenRestrictionAddress), 56, () => ({
					buffer: Buffer.concat([
						Buffer.of(0xA4, 0x78, 0xB2, 0x05, 0x04, 0x40, 0x38, 0x36), // tokenId
			...
			describe('supports token restriction global', () => {
				test.binary.test.addAll(getCodec(EntityType.tokenRestrictionGlobal), 42, () => ({
					buffer: Buffer.concat([
						Buffer.of(0x03, 0xC1, 0xC2, 0x33, 0xB2, 0xFF, 0x23, 0xAC), // tokenId
						Buffer.of(0x45, 0x32, 0x27, 0xAA, 0x23, 0xC2, 0x2B, 0xEE), // referenceTokenId
					...
					object: {
						tokenId: [0x33C2C103, 0xAC23FFB2],
						referenceTokenId: [0xAA273245, 0xEE2BC223],
